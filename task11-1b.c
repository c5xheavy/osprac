...
int main(void)
{
  ...
  if ((key = ftok(pathname, 0)) < 0) {
    printf("[error task11-1b.c]\tCan\'t generate key\n");
    exit(-1);
  }

  if ((msqid = msgget(key, 0666 | IPC_CREAT)) < 0) {
    printf("[error task11-1b.c]\tCan\'t get msqid\n");
    exit(-1);
  }
  ...
  while (1) {
    ...
    if (len = msgrcv(msqid, (struct msgbuf *) &mybuf, maxlen, 0, 0) < 0) {
      printf("[error task11-1b.c]\tCan\'t receive message from queue\n");
      exit(-1);
    }
    //
    ...
    printf("[success task11-1b.c]\tmessage type = %ld, sinfo = %i, finfo = %f\n", mybuf.mtype, mybuf.info.sinfo, mybuf.info.finfo);
  }
