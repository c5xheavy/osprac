...
int main()
{
    ...
    if ((key = ftok(pathname, 0)) < 0) {
        printf("[error task11-3-server.c]\tCan't generate key\n");
        exit(-1);
    }

    if ((msqid = msgget(key, 0666 | IPC_CREAT)) < 0) {
        printf("[error task11-3-server.c]\tCan't get msqid\n");
        exit(-1);
    }
    ...
    while (1) {
        ...
        if (len = msgrcv(msqid, (struct msgbuf *) &request_buf, maxlen, 1, 0) < 0) {
            printf("[error task11-3-server.c]\tCan't receive message from queue\n");
            exit(-1);
        }
        ...
        if (msgsnd(msqid, (struct msgbuf *) &response_buf, len, 0) < 0) {
            printf("[error task11-3-server.c]\tCan't send message to queue\n");
